...
const navItems: NavItem[] = [
  ...
  { title: 'Gestor de Posts', url: '/content/posts',   icon: Kanban },
      ...
      <SidebarHeader className="p-4">
        <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
          {collapsed ? (
            ...
            <img src={theme === 'dark' ? logoBranca : logoSvi} alt="SVI.Co" className="h-8 object-contain" />
          )}
        </div>
        {!collapsed && (
          <p className="text-xs text-muted-foreground mt-1 pl-1">Área do Cliente</p>
        )}
      </SidebarHeader>
        ...
        <div className={`flex items-center gap-2 px-2 py-2 rounded-lg bg-accent/50 ${collapsed ? 'justify-center' : ''}`}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium truncate">{profile?.name || 'Cliente'}</p>
