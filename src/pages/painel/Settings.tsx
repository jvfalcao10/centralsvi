...
import {
  ...
  ShieldCheck, CheckCircle2, XCircle, MessageCircle,
} from 'lucide-react'
...
function IntegrationsStatusCard({ integrations, isStaff }: { integrations: any[]; isStaff: boolean }) {
  const available = ['whatsapp', 'meta_ads', 'google_ads', 'google_analytics', 'kommo']
  return (
    <Card>
      <CardHeader>
...
function WhatsAppCard({ slug, integrations, onChanged }: { slug: string; integrations: any[]; onChanged: () => void }) {
  ...
  const [baseUrl, setBaseUrl] = useState(existing?.account_name || '')
  const [instanceToken, setInstanceToken] = useState('')
  const [token, setToken] = useState<string | null>(null)
  const [loading, setLoading] = useState(false)
  const appOrigin = typeof window !== 'undefined' ? window.location.origin : ''
  const webhookUrl = token ? `${appOrigin}/api/painel/whatsapp/${slug}?token=${token}` : null

  function copy(text: string) {
    navigator.clipboard.writeText(text); toast.success('Copiado')
  }
  ...
  async function save(e: React.FormEvent) {
    ...
    setToken(data.token)
    toast.success('UazAPI conectada. Configure o webhook URL abaixo na UazAPI.')
    onChanged()
  }
  ...
  return (
    <Card className="border-amber-300/40 dark:border-amber-800/40">
      <CardHeader>
        ...
        <CardDescription>
          {existing
            ? `Conectado: ${existing.account_name}. Pra reconfigurar, preencha e salve.`
            : 'Configure UazAPI pra esse cliente. Mensagens WhatsApp dos leads chegam no Inbox.'}
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {webhookUrl && (
          <div className="mt-4 p-3 rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 space-y-2">
            ...
            <div className="flex items-center justify-between gap-2">
              <code className="block text-xs font-mono break-all select-all flex-1">{webhookUrl}</code>
              <Button type="button" size="sm" variant="ghost" onClick={() => copy(webhookUrl)}><Copy className="w-3.5 h-3.5" /></Button>
            </div>
            <p className="text-xs text-muted-foreground">Salve agora — o token completo não aparece de novo.</p>
          </div>
        )}
      </CardContent>
    </Card>
