...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'POST') {
    ...
    // Só staff SVI pode configurar UazAPI (não cliente final)
    const { data: isSvi } = await userClient.rpc('painel_is_svi_team');
    if (!isSvi) return res.status(403).json({ error: 'forbidden', message: 'Só staff SVI configura UazAPI.' });

    const parsed = schema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: 'invalid_payload' });

    const { data: client } = await userClient.from('clients').select('id, slug').eq('slug', parsed.data.clientSlug).maybeSingle();
    ...
    const { error } = await admin.from('painel_integrations').upsert({
      ...
      credentials: {
        ...
        instance_token: parsed.data.instance_token,
    ...
    if (error) return res.status(400).json({ error: 'save_failed', message: error.message });
    return res.status(200).json({
      token,
      note: 'Salve este token. Configure-o como Webhook URL na UazAPI.',
    });
  }
  ...
  if (req.method === 'DELETE') {
    ...
    if (!user) return res.status(401).json({ error: 'unauthorized' });
    const { data: isSvi } = await userClient.rpc('painel_is_svi_team');
    if (!isSvi) return res.status(403).json({ error: 'forbidden' });

    const { clientSlug } = (req.body || {}) as any;
